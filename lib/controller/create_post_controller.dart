...
class CreatePostController extends GetxController {
  ...
  Future<ResponseModel> postImageData() async {
    ...
    try {
      var request = await http.MultipartRequest(
          "POST",
          Uri.parse(
              AppStrings.appBaseUrl + "api/mobile/newsfeed/picture/upload"));
  ...
  Future<ResponseModel> createPost(String imgLink) async {
    isLoading = true;
    update();
    Response response = await storyRepo.createPost(user.data!.user!.id.toString(), description.text.trim(), imgLink);
    ResponseModel responseModel;
    if (response.statusCode == 200) {
      if (!response.body["error"]) {
        DBManager().saveNewsFeedsData(response);
